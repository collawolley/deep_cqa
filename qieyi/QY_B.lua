--[[
	使用双样本的误差进行更新CNN，（两个样本计算误差，仅更新一个样本的参数）
	author:	liangjz
	time:	2015-03-23
--]]
--------------------
local Model = torch.class('QY_B')
function Model: __init(useGPU)
	...
	self.LM2 = {}
	self:getLM()	--生成语言模型
	self.dataSet = Yahoo('train',1)	--保险数据集，这里载入是为了获得测试集和答案
	self.dev = Yahoo('dev',1)	--每次只取一个负样本
	self.test = Yahoo('test',1)	--每次只取一个负样本
end	
-----------------------
...
function Model:getLM()	--获取语言模型
	...
	lm.sub = nn.CSubTable()
	for i=1,3 do
		self.LM2.emd[i]=self.LM.emd[i]:clone('weight','bias')
		self.LM2.rep[i]=self.LM.rep[i]:clone('weight','bias')
	end
	for i=1,2 do
		self.LM2.cosine[i]= self.LM.cosine[i]:clone('weight','bias')
	end
	self.LM2.sub= self.LM.sub:clone()
...
--		lm.linear[i]:share(lm.linear[1],'weight','bias','gradWeight','gradBias')
	...
	for i=1,3 do
		self.LM2.emd[i]:share(lm.emd[i],'weight','bias')
		self.LM2.rep[i]:share(lm.rep[i],'weight','bias')
	end
...
function Model:train(negativeSize)
	...
	self.dev:resetTrainset(negativeSize)
...
--		lm.linear[i]:share(lm.linear[1],'weight','bias','gradWeight','gradBias')
	...
	local sample =1	--占个坑
	local sample2= sample2 = self.dev:getNextPair()
	local index = {}	--存储字符下标
	local index2={}
	local vecs={}	--存储词向量
	local vecs2={}
	...
	while sample ~= nil do	--数据集跑完？	
		loop = loop + 1
		if loop %100 ==0 then xlua.progress(self.dataSet.current_train,#self.dataSet.questions) end
...
function Model:testOnePair(name,question_vec,answer_id) 	--给定一个问答pair，计算其相似度	
	local testSet=nil
	if name =='dev' then 
		testSet =self.dev
	else
	...
	local answer_rep = testSet:getAnswerVec(answer_id)	--获取答案的表达
	local sim_sc = self.LM.cosine[1]:forward({question_vec,answer_rep})
	return sim_sc[1]
end

function Model:evaluate(name)	--评估训练好的模型的精度，top 1是正确答案的比例
	...
	if name =='dev' then 
		testSet =self.dev
		test_size = #testSet.questions
	else
		testSet =self.test
		test_size = #testSet.questions
	end
	print('Calculating answers')
	local answer_pair =testSet:getNextAnswer(true)	--从头开始计算answer的向量
	while answer_pair~=nil do
		...
		testSet:saveAnswerVec(answer_pair[1],answer_rep)
		answer_pair = testSet:getNextAnswer()
	end	
	collectgarbage() 
	print('Test process:')
	local test_pair =nil
	test_pair = testSet:getNextTest(true)
	loop = 0
	while test_pair~=nil do
		loop = loop+1
		xlua.progress(loop,test_size)

		local gold = test_pair[2]	--正确答案的集合
		local qst = test_pair[1]	--问题
		...
		local score = self:testOnePair(name,qst_vec,gold)	--标准答案的得分,传入内容为问题的表达和答案的编号
		gold_sc[1] = score
		gold_rank[1] = 1	--初始化排名

		for k,c in pairs(candidates) do 
			local score = self:testOnePair(name,qst_vec,c)
		...
		test_pair = testSet:getNextTest()

		if loop%10==0 then collectgarbage() end
	end

	local results = torch.Tensor(results)
	local pr = torch.sum(results,1)/results:size()[1]
