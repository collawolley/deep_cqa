...
function train()
	...
	while sample ~= nil do	--数据集跑完？
		loop = loop + 1
		if loop %100 ==0 then xlua.progress(loop,data_set.negative_size*18540) end
		if next_sample then
...
data_set:resetTrainset()
--cfg.lm = torch.load('model/cov_1.bin','binary')
evaluate('dev')

train()
torch.save('model/cov_sdg2_2.bin',cfg.lm,'binary')
data_set:resetTrainset()
evaluate('dev')

train()
torch.save('model/cov_sdg2_3.bin',cfg.lm,'binary')
data_set:resetTrainset()
evaluate('dev')

train()
torch.save('model/cov_sdg2_4.bin',cfg.lm,'binary')
data_set:resetTrainset()
evaluate('dev')

train()
torch.save('model/cov_sdg2_5.bin',cfg.lm,'binary')
data_set:resetTrainset()
evaluate('dev')


train()
torch.save('model/cov_sdg2_6.bin',cfg.lm,'binary')
data_set:resetTrainset()
evaluate('dev')
