...
function testlm()	--应用修改模型后测试模型是否按照预期执行
	local lm = getlm()
	local criterion = nn.MarginCriterion(0.009)	--这个是magin的值
...
--[[	
	local hl = nn.Linear(cfg.dim,200):cuda()
	print(vec1:size())
	local t1 = hl:forward(vec1)
	t1 = nn.Replicate(1):cuda():forward(t1)
	print(t1:size())
	local cov = nn.SpatialConvolution(1,1000,200,2):cuda()	--input需要是3维tensor
	local t2 = cov:forward(t1)
	print(t2:size())
	print(t2[1000])
	local t3 = nn.SpatialAdaptiveMaxPooling(1,1):cuda():forward(t2)
	print(t3:size())
	print(t3[1000])
--]]
--[
