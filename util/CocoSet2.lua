...
--]]
local Coco = torch.class('CocoSet2')	--保险语料库的读取warper
function Coco:__init(negativeSize)
	self.corp = torch.load(deep_cqa.config.coco.corp2)
--	print(self.corp.testneg)
	self.trainSize = 10000 --#self.corp.trainid	--82783最多
...
function Coco:getNextPair()	--生成下一对问题-正样本-负样本对
	...
	local array = self.corp.trainid[self.current_train]	--语义相同的一组句子
	local qst = self.corp.sents[array[1]]--一个问题	总是选取第一个作为问题
	local true_id = array[self.current_answer]	--一个正确答案 选取其他作为答案	
	local false_id = deep_cqa.ins_meth.random_negative_id(array,#self.corp.sents,deep_cqa.config.random_seed)
	false_id = tonumber(false_id)
	deep_cqa.config.random_seed = (deep_cqa.config.random_seed +5)%50000000	--随机种子
	local false_answer = self.corp.sents[false_id]
	--print(self.corp.sents)
---	print(array,false_id,#self.corp.sents,false_answer)
...
function Coco:getNextAnswer(marker)	--返回下一个编号的答案
	print('function getNextAnswer disabled')
--[[
...
function Coco:getNextDev(marker)	--获取下一个验证集样本	【正确ids，问题，候选ids】
	...
	local candidate = {}
	for i =1,99 do 
		candidate[i] =tonumber(self.corp.testneg[self.dev_index-1][i])
	end
