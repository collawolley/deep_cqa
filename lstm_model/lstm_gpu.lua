...
-----------------------
	for i = 1 , train_set.size/20 do
		local feval = function (x)
			...
			local vecs ={}
			for j= 1,#sample do
				vecs[j] = get_embeddings(sample[j]):clone():cuda()
			...
			print(pred)
			local loss = criterion:forward(pred:cuda(),y)
			local obj_grad = criterion:backward(pred:cuda(),y)
