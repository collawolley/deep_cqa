...
function Coco:getIndex(sent) --	获取一个句子的索引表达，作为整个模型的输入，可以直接应用到词向量层
	return deep_cqa.read_one_sentence(sent,self.cfg.dict)
end
-----------------------

function Coco:getLM()	--获取语言模型
	self.LM ={}	--清空原始的模型
	local lm = self.LM
	local cfg = self.cfg
	lm.emd = {}
...
function Coco:testLM()	--应用修改模型后测试模型是否按照预期执行
...
--------------------------
function Coco:train(negativeSize)
...
function Coco:testOnePair(question_vec,answer_id) 	--给定一个问答pair，计算其相似度	
	...
	local answer_rep = self.LM.rep[2]:forward(ans_emd):clone()
	local sim_sc = self.LM.cosine[1]:forward({question_vec,answer_rep})
	return sim_sc[1]
end

function Coco:evaluate(name)	--评估训练好的模型的精度，top 1是正确答案的比例
	local results = {}	--记录每个测试样本的结果
	local test_size = 0
	local loop = 0
	if name =='dev' then 
		test_size = self.dataSet.valSize
	else
		test_size = self.dataSet.testSize
	end
	print('Test process:')
	local test_pair =nil
	if name =='dev' then
	...
	while test_pair~=nil do
		...
		local qst_idx = self:getIndex(self.dataSet:getAnswer(qst))
		...
		local gold_sc =nil
		local gold_rank = nil
	--	print('Qst:',self.dataSet:getAnswer(qst))
		gold_sc =  self:testOnePair(qst_vec,gold)	--标准答案的得分,传入内容为问题的表达和答案的编号
	--	print('True Answer:',string.format('%.5f',gold_sc),self.dataSet:getAnswer(gold))
		gold_rank = 1	--初始化排名

		for k,c in pairs(candidates) do 
			local score = self:testOnePair(qst_vec,c)
	--		print('Wrong Answer:',string.format('%.5f',score),self.dataSet:getAnswer(c))
			if score > gold_sc then
				gold_rank = gold_rank + 1
			end
		end
