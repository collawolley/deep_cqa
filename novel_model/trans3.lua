--[[
	CNN结构2的去掉问题部分的隐层,其他不变
	author:	liangjz
	time:	2015-01-19
--]]
--------------------
local Trans3 = torch.class('Trans3')
function Trans3: __init(useGPU)
	self.cfg = {	--配置文件
		vecs	= nil,
		dict	= nil,
		emd	= nil,
...
function Trans3:getIndex(sent) --	获取一个句子的索引表达，作为整个模型的输入，可以直接应用到词向量层
...
-------------------------------------
	...
	pt:add(nn.Tanh())
-------------------------------------
	local hlt =  nn.Linear(self.cfg.dim,self.cfg.dim)
...
function Trans3:testLM()	--应用修改模型后测试模型是否按照预期执行
...
--------------------------
function Trans3:train(negativeSize)
...
function Trans3:test_one_pair(question_vec,answer_id) 	--给定一个问答pair，计算其相似度	
...
function Trans3:evaluate(name)	--评估训练好的模型的精度，top 1是正确答案的比例
