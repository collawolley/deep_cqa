...
local LSTM, parent = torch.class('deep_cqa.LSTM', 'nn.Module')
		...
		self.output[t] = {}
      	for i = 1, self.num_layers do
			if reverse == true then
        		...
        		self.output[size-t+1][i] = htable[i]
			end
      end
    end
  end
...
function LSTM:share(lstm, ...)
  ...
  if self.cuda ~= lstm.cuda then error("LSTM cuda mismatch") end
