--[[
	尝试用一个隐层来给各个词的词向量赋予权重
	author:	liangjz
	time:	2015-01-20
--]]
--------------------
local Trans5 = torch.class('Trans5')
function Trans5: __init(useGPU)
	self.cfg = {	--配置文件
		vecs	= nil,
		dict	= nil,
		emd	= nil,
...
function Trans5:getIndex(sent) --	获取一个句子的索引表达，作为整个模型的输入，可以直接应用到词向量层
	return deep_cqa.read_one_sentence(sent,self.cfg.dict)
end
-----------------------

function Trans5:getLM()	--获取语言模型
	self.LM ={}	--清空原始的模型
	local qcov = nn.SpatialConvolution(1,1000,300,2,1,1,0,1)	--input需要是3维tensor
...
-------------------------------------
	...
	pt:add(nn.Tanh())
-------------------------------------
	local hlq = nn.CMul(self.cfg.dim)
	hlq['weight']= torch.ones(self.cfg.dim)
--.Sequential()
--lq:add(nn.CMul(self.cfg.dim))
--lq:add(nn.Tanh())
...
function Trans5:testLM()	--应用修改模型后测试模型是否按照预期执行
...
--------------------------
function Trans5:train(negativeSize)
	...
	while sample ~= nil do	--数据集跑完？
		...
		--if loop% 200 ==0 then print(tmp[1],tmp[2],tmp[3],tmp[4],tmp[5]) end
...
function Trans5:test_one_pair(question_vec,answer_id) 	--给定一个问答pair，计算其相似度	
...
function Trans5:evaluate(name)	--评估训练好的模型的精度，top 1是正确答案的比例
